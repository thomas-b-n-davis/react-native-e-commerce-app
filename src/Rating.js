...
export default class HomeScreen extends React.Component {
  ...
  loadData = async() => {
    ...
    this.setState({current:current,pid:current.id,uid:user.id,receiver_id:current.user_id});

    let self=this;
        axios.get(url+'/review/'+this.state.current.user_id)
		...
		return (	
          ...
          <TouchableOpacity style={{marginTop:10}} onPress={()=>this.props.navigation.navigate('DetailsScreen')}>
                <Image
                  ...
                  <Text style={{fontWeight:'bold',fontSize:29,color:'#ffffff',marginTop:15,flex:1}}>Reviews</Text>
